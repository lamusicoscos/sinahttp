...
static void async_sinahttp_log(struct evhttp_request *req, struct AsyncCallback *callback)
{
    ...
    req_time = sinahttp_ts_diff(callback->start_ts, end_ts);
    ...
    if (request_logging) {
        ...
        sinahttp_log(host_buf, req, req_time, id_buf, 0);
    }
}
                                ...
                                void (*finished_cb)(struct evhttp_request *, void *), void *finished_cb_arg)
{
    ...
    callback_group->id = sinahttp_request_id(req);
...
struct AsyncCallback *new_async_request_with_body(char *address, int port, char *path, char *body, void (*cb)(struct evhttp_request *, void *), void *cb_arg)
{
    ...
    if (evhttp_make_request(callback->evcon, callback->request, request_method, path) == -1) {
        ...
        async_sinahttp_log(callback->request, callback);
...
void finish_async_request(struct evhttp_request *req, void *cb_arg)
{
    ...
    // async_sinahttp_log handles this for us
    async_sinahttp_log(req, callback);
